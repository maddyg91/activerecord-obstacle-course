...
describe 'ActiveRecord Obstacle Course' do
  ...
  it '4. finds orders of multiple amounts' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    orders_of_500_and_700= Order.where(amount: [700, 500])
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    users = User.select(:name)
                .where(order_items:{item_id: item_8.id})
                .joins(:orders).joins("join order_items ON order_items.order_id=orders.id")
                .distinct
                .order(:name)
                .pluck(:name)
  ...
  custom_results = User.joins(:orders)
      .select(:name, "count(orders.id) as total_order_count")
      .group("users.name")
    # ---------------------------------------------------------------
    puts custom_results.map(&:attributes)
